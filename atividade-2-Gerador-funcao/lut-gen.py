...
for i in range(0,lut_len):
	...
	triang_lut += fmt.format(v = int(sig_ampl*(f*2) if f < 0.5 else -1*sig_ampl*(1-f*2)))
...
#print out results
print("static const uint8_t sine_lut[{l}] = \n {v};".format(l=lut_len,v=sin_lut))
print("static const uint8_t trng_lut[{l}] = \n {v};".format(l=lut_len,v=triang_lut))
print("static const uint8_t swtt_lut[{l}] = \n {v};".format(l=lut_len,v=sawtooth_lut))
